        ...
        const scrollnavSlick = () => {
          if (!media.matches) {
            if (!initialized) {
              ...
              // const visual = $(this.navSelector);
              // const progress = $('.scroll-nav-line');
              // const initPercent = 100 / $nav.find('.slick-slide').length;

              // progress.css('width', initPercent + '%');

              // visual.on(
              //   'beforeChange',
              //   function (event, slick, currentSlide, nextSlide) {
              //     var calc = ((nextSlide + 1) / slick.slideCount) * 100;
              //     progress.css('width', calc + '%').attr('aria-valuenow', calc);
              //   }
              // );
